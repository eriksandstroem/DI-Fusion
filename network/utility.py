...
def load_model(training_hyper_path: str, use_epoch: int = -1):
    ...
    if training_hyper_path.name.split(".")[-1] == "json":
        ...
        model_paths = {
            int(str(t).split("model_")[-1].split(".pth")[0]): t for t in model_paths
        }
        assert (
            use_epoch in model_paths.keys()
        ), f"{use_epoch} not found in {sorted(list(model_paths.keys()))}"
    ...
    if args.checkpoint is not None:
        ...
        if model.encoder is not None:
            state_dict = torch.load(
                Path(args.checkpoint).parent / f"encoder_{use_epoch}.pth.tar"
            )["model_state"]
...
def forward_model(
    model: nn.Module,
    network_input: torch.Tensor = None,
    ...
    if verbose:
        logging.debug(
            f"Network input chunks = {n_chunks}, each chunk = {network_input[0].size()}"
        )
        ...
        if loss_func is not None:
            ...
            loss_func(
                ...
                torch.arange(
                    ...
                    device=network_output[0].device,
                ),
...
def pack_samples(
    sample_indexer: torch.Tensor, count: int, sample_values: torch.Tensor = None
):
    """
    ...
    mapping, pinds, pcount = torch.unique(
        sample_indexer, return_inverse=True, return_counts=True
    )
    ...
    packed_inds_ind = torch.floor(
        torch.rand((n_batch, count), device=pcount.device) * pcount.unsqueeze(-1)
    ).long()  # (B, C)
    ...
    if sample_values is not None:
        ...
        packed = torch.index_select(sample_values, 0, packed_inds.view(-1)).view(
            n_batch, count, sample_values.size(-1)
        )
...
def groupby_reduce(
    sample_indexer: torch.Tensor, sample_values: torch.Tensor, op: str = "max"
):
    """
    ...
    assert n_samples == sample_values.size(
        0
    ), "Indexer and Values must agree on sample count!"
