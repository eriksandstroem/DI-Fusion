...
class CORBSSequence(RGBDSequence):
    ...
    def __len__(self):
        return 2 * len(self.poses_matched.keys())

    def __next__(self):
        if self.frame_id >= len(self):
            raise StopIteration
        # I need some idea to pick when frame_id is even tof and when it is odd stereo depth while selecting the same time stamp pose for each of them.
        timestamp_pose = list(self.poses_matched.keys())[self.frame_id // 2]
        timestamp_rgb = self.pose_to_rgb[timestamp_pose]
        timestamp_depth = self.pose_to_depth[timestamp_pose]

        # specify intrinsics
        if self.frame_id % 2 == 0:  # str(depth_img_path).endswith(".png"):  # ToF
            calib = [
                525.0 * 256 / 640,
                525.0 * 256 / 480,
                319.5 * 256 / 640,
