...
class OptimizeProcess(mp.Process):
    ...
    def run(self):
        ...
        while True:
            ...
            job_kwargs = {k: self._move_args(v) for k, v in new_job_kwargs.items()}
            del new_job_kwargs
    ...
    def do_optimize(
        ...
        def loss_func(net_output, output_inds):
            ...
            ll_loss = -torch.distributions.Normal(loc=pd_sdf, scale=pd_std).log_prob(
                gt_sdf
            )
        ...
        for optim_iter in range(args.optim_n_iters):
            ...
            _ = net_util.forward_model(
                decoder,
                latent_input=latent_vecs,
                xyz_input=gathered_relative_xyz,
                loss_func=loss_func,
                max_sample=int(1.5e6),
                verbose=(optim_iter == 0),
            )
            optimizer.step()
            batch_loss.clear()
...
class DenseIndexedMap:
    def __init__(
        ...
        self.n_xyz = (
            np.ceil(
                (np.asarray(args.bound_max) - np.asarray(args.bound_min))
                / args.voxel_size
            )
            .astype(int)
            .tolist()
        ...
        logging.info(
            f"Map size Nx = {self.n_xyz[0]}, Ny = {self.n_xyz[1]}, Nz = {self.n_xyz[2]}"
        )

        self.args = args
        self.bound_min = torch.tensor(args.bound_min, device=device).float()
        self.bound_max = self.bound_min + self.voxel_size * torch.tensor(
            self.n_xyz, device=device
        )
        self.latent_dim = latent_dim
        self.device = device
        self.integration_offsets = [
        ...
        self.relative_network_offset = torch.tensor(
            [[0.5, 0.5, 0.5]], device=self.device, dtype=torch.float32
        )

        self.cold_vars = {
            "n_occupied": 0,
            "indexer": torch.ones(
                np.product(self.n_xyz), device=device, dtype=torch.long
            )
            * -1,
            # -- Voxel Attributes --
            # 1. Latent Vector (Geometry)
            "latent_vecs": torch.empty(
                (1, self.latent_dim), dtype=torch.float32, device=device
            ),
            # 2. Position
        ...
        if enable_async:
            self.optimize_process = OptimizeProcess(
                self.model.decoder,
    ...
    def _get_var(self, name):
        if (
            threading.get_ident() == self.meshing_thread_id
            and name in self.backup_var_names
        ):
    ...
    def _inflate_latent_buffer(self, count: int):
        ...
        if self.latent_vecs.size(0) < target_n_occupied:
            ...
            while new_size < target_n_occupied:
                new_size *= 2
            new_vec = torch.empty(
                (new_size, self.latent_dim), dtype=torch.float32, device=self.device
            )
            new_vec[: self.latent_vecs.size(0)] = self.latent_vecs
            new_vec_pos = (
                torch.ones((new_size,), dtype=torch.long, device=self.device) * -1
            )
            new_vec_pos[: self.latent_vecs.size(0)] = self.latent_vecs_pos
            new_voxel_conf = torch.zeros(
                (new_size,), dtype=torch.float32, device=self.device
            )
            new_voxel_conf[: self.latent_vecs.size(0)] = self.voxel_obs_count
            new_voxel_optim = torch.zeros(
                (new_size,), dtype=torch.bool, device=self.device
            )
        ...
        new_inds = torch.arange(
            self.n_occupied, target_n_occupied, device=self.device, dtype=torch.long
        )
    ...
    def _linearize_id(self, xyz: torch.Tensor):
        ...
        return (
            xyz[:, 2]
            + self.n_xyz[-1] * xyz[:, 1]
            + (self.n_xyz[-1] * self.n_xyz[-2]) * xyz[:, 0]
        )
    ...
    def _mark_updated_vec_id(self, new_vec_id: torch.Tensor):
        ...
        """
        self.mesh_cache.updated_vec_id = torch.cat(
            [self.mesh_cache.updated_vec_id, new_vec_id]
        )
    ...
    def _update_optimize_result_set(self, deintegrate_old: bool):
        ...
        if not deintegrate_old:
            ...
            original_count = self.optimize_result_set.old_latent_obs_counts.unsqueeze(
                -1
            )
            delta_vecs_sum = (
                self.latent_vecs[idx] * cur_count
                + (
                    self.optimize_result_set.new_latent_vecs
                    - self.optimize_result_set.old_latent_vecs
                )
                * original_count
            )
    ...
    def integrate_keyframe(
        ...
        if not self.optimize_process.res_queue.empty():
            self.optimize_result_set.new_latent_vecs = (
                self.optimize_process.res_queue.get()
            )
        ...
        if self.args.prune_min_vox_obs > 0:
            _, unq_inv, unq_count = torch.unique(
                surface_grid_id, return_counts=True, return_inverse=True
            )
        ...
        try:
            ...
            if invalid_surface_ind.sum() > 0:
                ...
                invalid_flatten_id = self._expand_flatten_id(
                    invalid_flatten_id, ensure_valid=False
                )
                invalid_flatten_id = invalid_flatten_id[
                    self.indexer[invalid_flatten_id] == -1
                ]
        ...
        map_status = torch.zeros(
            np.product(self.n_xyz), device=self.device, dtype=torch.short
        )

        encoder_voxel_pos = self.latent_vecs_pos[
            torch.logical_and(
                self.voxel_obs_count < self.args.encoder_count_th,
        ...
        if encoder_voxel_pos.size(0) > 0:
            ...
            for offset in self.integration_offsets:
                _surface_grid_id = (
                    torch.ceil(pruned_surface_xyz_normalized + offset) - 1
                )
                for dim in range(3):
                    _surface_grid_id[:, dim].clamp_(0, self.n_xyz[dim] - 1)
                surface_relative_xyz = (
                    pruned_surface_xyz_normalized
                    - _surface_grid_id
                ...
                gathered_surface_latent_inds.append(
                    surface_latent_ind[in_focus_obs_mask]
                )
                gathered_surface_xyzn.append(
                    torch.cat(
                        [
            ...
            surface_blatent_mapping, pinds, pcounts = torch.unique(
                gathered_surface_latent_inds, return_inverse=True, return_counts=True
            )
            pcounts = pcounts.float()

            try:
                logging.info(
                    f"{surface_blatent_mapping.size(0)} voxels will be updated by the encoder. "
            ...
            encoder_latent_sum = net_util.groupby_reduce(
                pinds, encoder_latent, op="sum"
            )  # (C, L)
            encoder_latent_sum += self.latent_vecs[
                surface_blatent_mapping
            ] * self.voxel_obs_count[surface_blatent_mapping].unsqueeze(-1)
            self.voxel_obs_count[surface_blatent_mapping] += pcounts
            self.latent_vecs[
                surface_blatent_mapping
            ] = encoder_latent_sum / self.voxel_obs_count[
                surface_blatent_mapping
            ].unsqueeze(
                -1
            )
        ...
        if (
            do_optimize
            and (
                not self.optimize_process.is_busy()
                and self.optimize_process.res_queue.empty()
            )
            and self.args.optim_n_iters > 0
        ):
            optim_voxel_pos = self.latent_vecs_pos[
                torch.logical_and(
                    self.voxel_obs_count >= self.args.encoder_count_th,
            ...
            if optim_voxel_pos.size(0) > 0:
                ...
                (
                    pruned_surface_xyz_normalized,
                    pruned_surface_normal,
                ) = get_pruned_surface(enabled=True, lin_pos=optim_voxel_pos)
                ...
                latent_id_subset_uniques, latent_id_inv_mapping = torch.unique(
                    gathered_latent_inds, return_inverse=True
                )
                ...
                if not async_optimize:
                    self.optimize_result_set.new_latent_vecs = (
                        self.optimize_process.do_optimize(
                            decoder=self.model.decoder, **optimize_kwargs
                        )
                    )
                    self._update_optimize_result_set(deintegrate_old=False)
                else:
                    self.optimize_result_set.old_latent_vecs = torch.clone(
                        latent_vecs_unique
                    )
                    self.optimize_result_set.old_latent_obs_counts = (
                        self.voxel_obs_count[latent_id_subset_uniques]
                    )
    ...
    def _expand_flatten_id(
        self, base_flatten_id: torch.Tensor, ensure_valid: bool = True
    ):
    ...
    def get_sdf(self, xyz: torch.Tensor):
        ...
        with torch.no_grad():
            ...
            # valid_valid_mask = (
            #     self.voxel_obs_count[sample_latent_id[sample_valid_mask]]
            #     > self.args.ignore_count_th
            # )
            # sample_valid_mask[sample_valid_mask.clone()] = valid_valid_mask
            # latent = self.latent_vecs.cpu()
            # s_v_m = sample_valid_mask.cpu()
            # s_l_id = sample_latent_id.cpu()
            # valid_latent = latent[s_l_id[s_v_m]]
            # valid_latent = self.latent_vecs[sample_latent_id[sample_valid_mask]]
            valid_latent = self.latent_vecs[sample_latent_id[sample_valid_mask]]

        valid_xyz_rel = (
            xyz_normalized[sample_valid_mask]
            - grid_id[sample_valid_mask]
            - self.relative_network_offset
        )

        sdf, std = net_util.forward_model(
            self.model.decoder,
            latent_input=valid_latent,
            xyz_input=valid_xyz_rel,
            no_detach=True,
        )
        print(sdf.shape)
    ...
    def extract_mesh(
        ...
        def do_meshing(voxel_resolution):
            ...
            with torch.cuda.stream(self.meshing_stream):
                ...
                occupied_vec_id = occupied_vec_id[
                    self.voxel_obs_count[occupied_vec_id] > self.args.ignore_count_th
                ]

                vec_id_batch_mapping = (
                    torch.ones(
                        (occupied_vec_id.max().item() + 1,),
                        device=self.device,
                        dtype=torch.int,
                    )
                    * -1
                )
                vec_id_batch_mapping[occupied_vec_id] = torch.arange(
                    0, occupied_vec_id.size(0), device=self.device, dtype=torch.int
                )
                occupied_latent_vecs = self.latent_vecs[occupied_vec_id]  # (B, 125)
                B = occupied_latent_vecs.size(0)

                # Sample more data.
                sample_a = -(voxel_resolution // 2) * (1.0 / voxel_resolution)
                sample_b = 1.0 + (voxel_resolution - 1) // 2 * (1.0 / voxel_resolution)
                voxel_resolution *= 2

                low_resolution = voxel_resolution // 2 if fast else voxel_resolution
                low_samples = (
                    net_util.get_samples(
                        low_resolution, self.device, a=sample_a, b=sample_b
                    )
                    - self.relative_network_offset
                )  # (l**3, 3)
                low_samples = low_samples.unsqueeze(0).repeat(B, 1, 1)  # (B, l**3, 3)
                low_latents = occupied_latent_vecs.unsqueeze(1).repeat(
                    1, low_samples.size(1), 1
                )  # (B, l**3, 3)

                with torch.no_grad():
                    low_sdf, low_std = net_util.forward_model(
                        self.model.decoder,
                ...
                if fast:
                    low_sdf = low_sdf.reshape(
                        B, 1, low_resolution, low_resolution, low_resolution
                    )  # (B, 1, l, l, l)
                    low_std = low_std.reshape(
                        B, 1, low_resolution, low_resolution, low_resolution
                    )
                    high_sdf = torch.nn.functional.interpolate(
                        low_sdf,
                        mode="trilinear",
                        size=(voxel_resolution, voxel_resolution, voxel_resolution),
                        align_corners=True,
                    )
                    high_std = torch.nn.functional.interpolate(
                        low_std,
                        mode="trilinear",
                        size=(voxel_resolution, voxel_resolution, voxel_resolution),
                        align_corners=True,
                    )
                    high_sdf = high_sdf.squeeze(0).reshape(
                        B, voxel_resolution ** 3
                    )  # (B, H**3)
                    ...
                    if high_valid_lifs.size(0) > 0:
                        high_samples = (
                            net_util.get_samples(
                                voxel_resolution, self.device, a=sample_a, b=sample_b
                            )
                            - self.relative_network_offset
                        )  # (H**3, 3)
                        high_latents = occupied_latent_vecs[
                            high_valid_lifs
                        ]  # (VH, 125)
                        high_samples = high_samples[high_valid_sbs]  # (VH, 3)

                        with torch.no_grad():
                            high_valid_sdf, high_valid_std = net_util.forward_model(
                                self.model.decoder,
                                latent_input=high_latents,
                                xyz_input=high_samples,
                            )
                        high_sdf[
                            high_valid_lifs, high_valid_sbs
                        ] = high_valid_sdf.squeeze(-1)
                        high_std[
                            high_valid_lifs, high_valid_sbs
                        ] = high_valid_std.squeeze(-1)

                    high_sdf = high_sdf.reshape(
                        B, voxel_resolution, voxel_resolution, voxel_resolution
                    )
                    high_std = high_std.reshape(
                        B, voxel_resolution, voxel_resolution, voxel_resolution
                    )
                else:
                    high_sdf = low_sdf.reshape(
                        B, low_resolution, low_resolution, low_resolution
                    )
                    high_std = low_std.reshape(
                        B, low_resolution, low_resolution, low_resolution
                    )

                high_sdf = -high_sdf
                # print(focused_flatten_id.shape)
                # print(vec_id_batch_mapping.shape)
                # print(high_sdf.shape)
                #
                # occupied_flatten_id = torch.where(self.indexer != -1)[0]  # (B, )
                # blk_xyz = self._unlinearize_id(occupied_flatten_id)
                # print(blk_xyz.shape)

                # sdf = self.get_sdf(blk_xyz)
                # print(sdf[0].shape)
                ...
                if interpolate:
                    (
                        ...
                        vertices_flatten_id,
                        vertices_std,
                    ) = system.ext.marching_cubes_interp(
                ...
                if self.mesh_cache.vertices is None:
                    ...
                    valid_verts_idx = _get_valid_idx(
                        self.mesh_cache.vertices_flatten_id, p
                    )
                    self.mesh_cache.vertices = np.concatenate(
                        [self.mesh_cache.vertices[valid_verts_idx], vertices], axis=0
                    )
                    self.mesh_cache.vertices_flatten_id = np.concatenate(
                        [
                            self.mesh_cache.vertices_flatten_id[valid_verts_idx],
                            vertices_flatten_id,
                        ],
                        axis=0,
                    )
                    self.mesh_cache.vertices_std = np.concatenate(
                        [self.mesh_cache.vertices_std[valid_verts_idx], vertices_std],
                        axis=0,
                    )

        if extract_async:
            self.meshing_thread = threading.Thread(
                target=do_meshing, args=(voxel_resolution,)
            )
    ...
    def get_fast_preview_visuals(self):
        ...
        blk_edges = []
        for vert_offset in [
            [0.0, 0.0, self.voxel_size],
            [0.0, self.voxel_size, 0.0],
            [0.0, self.voxel_size, self.voxel_size],
            [self.voxel_size, 0.0, 0.0],
            [self.voxel_size, 0.0, self.voxel_size],
            [self.voxel_size, self.voxel_size, 0.0],
            [self.voxel_size, self.voxel_size, self.voxel_size],
        ]:
            blk_verts.append(
                blk_verts[0]
                + torch.tensor(
                    vert_offset, dtype=torch.float32, device=blk_verts[0].device
                ).unsqueeze(0)
            )
        for vert_edge in [
            [0, 1],
            ...
            blk_edges.append(
                np.stack(
                    [
                        np.arange(n_block, dtype=np.int32) + vert_edge[0] * n_block,
                        np.arange(n_block, dtype=np.int32) + vert_edge[1] * n_block,
                    ],
                    axis=1,
                )
            )
        ...
        return [
            blk_wireframe,
            vis_util.wireframe_bbox(
                self.bound_min.cpu().numpy(), self.bound_max.cpu().numpy(), color_id=4
            ),
        ]
    ...
    def get_map_visuals(
        ...
        if return_blocks:
            ...
            for blk_start_i, ofid in zip(blk_start, occupied_flatten_id):
                if ofid in self.debug_show_blocks:
                    blk_wireframes.append(
                        vis_util.wireframe_bbox(
                            blk_start_i, blk_start_i + blk_dim, solid=True, color_id=4
                        )
                    )
                else:
                    blk_wireframes.append(
                        vis_util.wireframe_bbox(
                            blk_start_i, blk_start_i + blk_dim, solid=False
                        )
                    )

            blk_wireframes.append(
                vis_util.wireframe_bbox(
                    self.bound_min.cpu().numpy(),
                    self.bound_max.cpu().numpy(),
                    color_id=4,
                )
            )
        ...
        if return_mesh:
            map_visuals.mesh = [
                self.extract_mesh(voxel_resolution, int(1e7), extract_async=False)
            ]
        ...
        if return_samples or return_uncertainty:
            ...
            high_samples = (
                net_util.get_samples(voxel_resolution, self.device)
                - self.relative_network_offset
            )  # (H**3, 3)
            high_samples = high_samples.unsqueeze(0).repeat(B, 1, 1)  # (B, H**3, 3)
            high_latents = occupied_latent_vecs.unsqueeze(1).repeat(
                1, high_samples.size(1), 1
            )  # (B, H**3, 125)

            with torch.no_grad():
                high_sdf, high_uncertainty = net_util.forward_model(
                    self.model.decoder,
            ...
            high_uncertainty = high_uncertainty.reshape(
                B, voxel_resolution, voxel_resolution, voxel_resolution
            )
            ...
            vis_sample_pos = (
                high_samples
                + vis_grid_base.unsqueeze(1).repeat(1, high_samples.size(1), 1)
                + self.relative_network_offset
            )
            vis_sample_pos = (
                (vis_sample_pos.reshape(-1, 3) * self.voxel_size + self.bound_min)
                .cpu()
                .numpy()
            )
            ...
            if sample_range is None:
                vis_high_sdf = (high_sdf - high_sdf.min()) / (
                    high_sdf.max() - high_sdf.min()
                )
                vis_std = (high_uncertainty - high_uncertainty.min()) / (
                    high_uncertainty.max() - high_uncertainty.min()
                )
                print(
                    f"Uncertainty normalized to {high_uncertainty.min().item()} ~ {high_uncertainty.max().item()}"
                )
            else:
                vis_high_sdf = (high_sdf - sample_range[0]) / (
                    sample_range[1] - sample_range[0]
                )
                vis_std = (high_uncertainty - sample_range[0]) / (
                    sample_range[1] - sample_range[0]
                )
            ...
            if return_samples:
                map_visuals.samples = [
                    vis_util.pointcloud(vis_sample_pos, cfloat=vis_high_sdf)
                ]
            if return_uncertainty:
                map_visuals.uncertainty = [
                    vis_util.pointcloud(vis_sample_pos, cfloat=vis_std)
                ]
