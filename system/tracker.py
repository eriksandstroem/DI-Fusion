...
def point_box_filter(points: torch.Tensor, normals: torch.Tensor, voxel_size: float):
    ...
    n_x, n_y, n_z = (
        (torch.floor((max_bound - min_bound) / voxel_size).long() + 16)
        .cpu()
        .numpy()
        .tolist()[0]
    )
...
class SDFTracker:
    ...
    def _make_image_pyramid(self, intensity_img: torch.Tensor, depth_img: torch.Tensor):
        ...
        d1_intensity = torch.nn.functional.interpolate(
            d0_intensity, (d1_h, d1_w), mode="bilinear"
        )
        d1_depth = torch.nn.functional.interpolate(
            d0_depth, (d1_h, d1_w), mode="nearest"
        )
        d2_intensity = torch.nn.functional.interpolate(
            d1_intensity, (d2_h, d2_w), mode="bilinear"
        )
        d2_depth = torch.nn.functional.interpolate(
            d1_depth, (d2_h, d2_w), mode="nearest"
        )
        ...
        return (
            [
                ...
                for t in [d0_intensity, d1_intensity, d2_intensity]
            ],
    ...
    def track_camera(
        ...
        with torch.cuda.device(self.map.device):
            # remove heuristic outlier filtering of the depth maps
        ...
        if set_pose is not None:
            ...
            lspeed = Isometry()
            final_pose = self.gauss_newton(
                self.all_pd_pose[-1].dot(lspeed),
                cur_intensity,
                cur_depth,
                cur_dIdxy,
                pc_data,
    ...
    def compute_rgb_Hg(
        ...
        cur_intensity_pyramid: list,
        cur_depth_pyramid: list,
        cur_dIdxy_pyramid: list,
        calib: FrameIntrinsic,
        no_grad: bool = False,
    ):
        ...
        odometry_output = rgb_odometry(
            self.last_intensity[pyramid_level],
            self.last_depth[pyramid_level],
            cur_intensity_pyramid[pyramid_level],
            cur_depth_pyramid[pyramid_level],
            cur_dIdxy_pyramid[pyramid_level],
            [calib.fx, calib.fy, calib.cx, calib.cy],
            KRKinv.flatten().tolist(),
            Kt.flatten().tolist(),
            self.rgb_args.min_grad_scale,
            self.rgb_args.max_depth_delta,
            not no_grad,
        )
        if no_grad:
        ...
        if J_map is not None:
            J_map = -J_map[
                f_valid_mask
            ]  # (M, 6), The derivative computed is actually for -xi, so we inverse it.
            JW = J_map

        if self.rgb_args.robust_kernel is not None:
            term_weight = self._robust_kernel(
                f_map, self.rgb_args.robust_kernel, self.rgb_args.robust_k
            )
            Wf = Wf * term_weight  # (M)
            JW = JW * term_weight.unsqueeze(1) if JW is not None else None

        error_scale = 1.0 / Wf.size(0) * self.rgb_args.weight
        ...
        if not no_grad:
            ...
            return (
                H.cpu().numpy().astype(float),
                g.cpu().numpy().astype(float),
                float(sum_error),
            )
        else:
    ...
    def compute_sdf_Hg(
        ...
        if no_grad:
            ...
            Lt = torch.from_numpy(
                last_pose.q.rotation_matrix.astype(np.float32).T
            ).cuda()
        ...
        if self.sdf_args.robust_kernel is not None:
            term_weight = self._robust_kernel(
                cur_obs_sdf, self.sdf_args.robust_kernel, self.sdf_args.robust_k
            )
            Wf = Wf * term_weight  # (M)
            JW = JW * term_weight.unsqueeze(1) if JW is not None else None
    ...
    def gauss_newton(
                ...
                for loss_config in group_iter_config["type"]:
                    if loss_config[0] == "sdf":
                        sdf_H, sdf_g, sdf_energy = self.compute_sdf_Hg(
                            i_iter, last_pose, cur_delta_pose, obs_xyz, i_iter == -1
                        )
        ...
        if i_iter >= 10:
            ...
            if self.n_unstable >= 3:
                self.rgb_args.weight = max(self.rgb_args.weight, 500.0)
    ...
    def track_camera_points_lm(self, init_pose: Isometry, obs_xyz: torch.Tensor):
        ...
        for i_iter in range(self.args.n_gn_iter):
            f, dsdf_dxi = self.get_error_func(
                cur_pose, obs_xyz, need_grad=True
            )  # (M, ), (M, 6)
            ...
            if self.args.robust_kernel:
                ...
                term_weight = torch.where(
                    obs_sdf_abs <= self.args.robust_k,
                    term_weight,
            ...
            H = (
                torch.einsum("nx,ny->xy", dsdf_dxi * term_weight.unsqueeze(1), dsdf_dxi)
                .cpu()
                .numpy()
            )  # (6, 6)
            ...
            if self.args.robust_kernel:
                ...
                new_weight = torch.where(
                    f_abs <= self.args.robust_k, new_weight, self.args.robust_k / f_abs
                )

            rho = (
                (f * Wf.squeeze()).sum() - (f_new * new_weight * f_new).sum()
            ).item() / rho_denom
