...
def refresh(vis):
    ...
    if not vis_param.mesh_updated and vis_param.args.run_async:
        map_mesh = vis_param.map.extract_mesh(
            vis_param.args.resolution, 0, extract_async=True
        )
    ...
    if vis_param.sequence.frame_id >= len(vis_param.sequence):
        if vis:
            ...
            map_mesh = vis_param.map.extract_mesh(
                vis_param.args.resolution,
                int(1.584e7),
                max_std=0.06,
                extract_async=vis_param.args.run_async,
                interpolate=True,
            )

            # crop the mesh here to fit better.
            # map_mesh.crop()
            # http://www.open3d.org/docs/0.9.0/python_api/open3d.geometry.TriangleMesh.html#open3d.geometry.TriangleMesh
            ...
            o3d.io.write_triangle_mesh(
                "/cluster/work/cvl/esandstroem/3rdparty/di_fusion/copyroom_tof_std0.06.ply",
                map_mesh,
            )
    ...
    if frame_data.calib.dscale == 1.0:  # for Scene3D MVS
        frame_data.depth[
            torch.logical_or(
                frame_data.depth < 0.5,
                frame_data.depth > 3.0,
            )
        ] = np.nan
    else:
        frame_data.depth[
            torch.logical_or(
                frame_data.depth < vis_param.args.depth_cut_min,
                frame_data.depth > vis_param.args.depth_cut_max,
            )
        ] = np.nan

    # Do tracking.
    # print(frame_data.gt_pose)
    frame_pose = vis_param.tracker.track_camera(
        frame_data.rgb,
        frame_data.depth,
        frame_data.calib,
        # vis_param.sequence.first_iso
        # if len(vis_param.tracker.all_pd_pose) == 0
        # else None,
        frame_data.gt_pose,
    )
    ...
    if vis:
        ...
        update_geometry(
            vis_util.trajectory([t.t for t in vis_param.tracker.all_pd_pose]),
            "traj_geometry",
            vis,
        )
        update_geometry(
            vis_util.camera(frame_pose, scale=0.15, color_id=3), "camera_geometry", vis
        )
    ...
    if (vis_param.sequence.frame_id - 1) % vis_param.args.integrate_interval == 0:
        ...
        vis_param.map.integrate_keyframe(
            opt_depth,
            opt_normal,
            async_optimize=vis_param.args.run_async,
            do_optimize=False,
        )
            ...
            update_geometry(
                (
                    vis_util.wireframe_bbox(
                        vis_param.map.bound_min.cpu().numpy(),
                        vis_param.map.bound_max.cpu().numpy(),
                        color_id=4,
                    )
                ),
                "bound_box",
...
if __name__ == "__main__":
    ...
    if torch.cuda.device_count() > 1:
        main_device, aux_device = torch.device("cuda", index=0), torch.device(
            "cuda", index=1
        )
    ...
    vis_param.map = map.DenseIndexedMap(
        model,
        args.mapping,
        args.model.code_length,
        main_device,
        args.run_async,
        aux_device,
    )
    ...
    if args.vis:
        ...
        engine.create_window(
            window_name="Implicit SLAM", width=1280, height=720, visible=True
        )
        ...
        vis_ph = vis_util.wireframe_bbox([-4.0, -4.0, -4.0], [4.0, 4.0, 4.0])
